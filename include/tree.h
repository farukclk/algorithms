...
// In-order traversal
TreeNode *create_BT(int dizi[], int length) {
    ...
    if (length == 1) {
        ...
        root->left = create_BT(sol_dizi, sol_index);
        root->right = create_BT(sag_dizi, sag_index);
...
int size_of_BT(TreeNode *root) {
    ...
    size += size_of_BT(root->left);
    size += size_of_BT(root->right);
